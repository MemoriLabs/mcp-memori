...
import { z } from "zod";
const MEMORI_API_URL = process.env.MEMORI_API_URL || "https://api.memorilabs.ai/mcp";
...
async function getRemoteClient() {
    ...
    const transport = new StreamableHTTPClientTransport(new URL(MEMORI_API_URL), { requestInit: { headers } });
...
server.registerTool("recall", {
    ...
    description: "Retrieve relevant memories for a given query. Call at the start of user turns to fetch prior context, preferences, and facts.",
    inputSchema: {
        query: z
            .string()
    ...
    try {
        ...
        return {
            content: [
                { type: "text", text: `Error recalling memories: ${message}` },
            ],
            isError: true,
        };
    }
...
server.registerTool("advanced_augmentation", {
    title: "Store Memory",
    description: "Store durable facts and preferences after drafting a response. Call after responding to persist user context across sessions.",
    inputSchema: {
